...
produce.heatmaps<-function(data,    #data.norm.filt
        ...
        for (c in 1:length(controls)){
                ...
                for (i in 1:length(annotContMatrix)) {
                        #adjust p-values from Bayes fit using FDR method
                        adjPVAL<-p.adjust(fit2Eb$p.value[,annotContMatrix[i]],method="BH")
                        ...
                        logFoldChange<-fit2Eb$coefficients[,annotContMatrix[i]]
                ...
                phenData<-pData(data)
                samples.sel<-c(unlist(controls[c]),unlist(treatments[c]))
                ...
                for (i in 1:(length(dataList.for.heatmap[[4]]))){
                        ...
                        print(rowLabels)
                        print(dataList.for.heatmap[[4]][[i]])
                        Rowv<-ifelse(i==1,TRUE,FALSE)
                        dendrogram<-ifelse(i==1,"both","column")
                        
                        file.name=paste0("heatmap-control#",c,"-",
