#modified vennDiagram and vennCounts functions
#2015-03-13
#added cexCounts (adjustment of numbers separate from the rest of the text)
#modified showCounts function to include cexCounts and thus implement
#separate cex for text and numbers

vennDiagram.mod<-function (object, include = "both", names = NULL, mar = rep(1,4), 
                           cex = c(1.5, 1, 0.7), cexCounts,
                           ...
                           show.include = NULL, ...) 
{
        ...
        if (is(object, "VennCounts")) {
                ...
                if (LenInc > 1) 
                        z2 <- vennCounts.mod(object, include = include[2])[, 
                                                                       "Counts"]
                object <- vennCounts.mod(object, include = include[1])
        }
        z <- object[, "Counts"]
        nsets <- ncol(object) - 1
        if (nsets > 5) 
        ...
        if (nsets <= 3) {
                ...
                showCounts <- switch(nsets, function(counts, cex, adj, 
                        ...
                        text(-1.5, 0.1, counts[3], cex = cexCounts, col = col, 
                             adj = adj)
                        ...
                        text(0.75, -0.35, counts[4], cex = cexCounts, col = col, 
                             adj = adj)
                        text(-1.5, 1, counts[5], cex = cexCounts, col = col, adj = adj)
                        text(-0.75, -0.35, counts[6], cex = cexCounts, col = col, 
                             adj = adj)
                        text(0, 0.9, counts[7], cex = cexCounts, col = col, adj = adj)
                        text(0, 0, counts[8], cex = cexCounts, col = col, adj = adj)
                ...
                showCounts(counts = z, cex = cex, adj = adj, col = counts.col[1], 
                           leg = include[1])
...
vennCounts.mod<-function (x, include = "BOTH") 
{
        x <- as.matrix(x)
